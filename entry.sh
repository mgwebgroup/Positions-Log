...
header_fields="TIMESTAMP,INSTRUMENT,QUANTITY,PRICE,CURRENCY,COMMISSION,EXCHANGE_EXEC_ID,ORDER_ID,ACCOUNT_NO"
...
while : ; do
	...
	while : ; do
		read -e -p COMMISSION: COMMISSION
	...
	# 	( source meta_data_manager.sh )
	# fi

	# this will adjust quantities in data cache for DDRU and META records only if we subtracted from a position
	# adjust_cache
